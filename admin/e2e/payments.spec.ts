...
test('an admin posts a cash out with a receipt, finds it in the cash book and reverses it; the balance follows', async ({ page }) => {
  ...
  await form.getByLabel('Amount (BDT)').fill('35000')
  await expect(form.getByRole('button', { name: 'Post to ledger' })).toBeDisabled()
  await form.getByLabel('Attach the receipt, bank slip or screenshot').setInputFiles(PHOTO)
  await form.getByRole('button', { name: 'Post to ledger' }).click()
  ...
  await expect(row).toContainText('− BDT 35,000')
...
test('a deal with an advance shows what is due, takes the rest and is marked paid', async ({ page }) => {
  ...
  await expect(form).toContainText('Invoice for BDT 2,00,000: BDT 50,000 received now, BDT 1,50,000 due.')
  ...
  await expect(deal).toContainText('BDT 1,50,000')
